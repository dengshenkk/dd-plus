...
import { clear, run, sleep, withTaskName } from './utils'
import { outDir, projectRoot } from './utils/paths'
import path from 'path'
import ts from 'gulp-typescript'

export function buildPackages (dirName: string, packageName: string) {
	...
	const tasks = Object.entries(buildConfig).map(([ module, config ]) => {
		...
		return series(
			...
			withTaskName('copy:' + module, async () => {
				return src(`${ output }/**`)
...
// 						dest(
// 							path.resolve(
// 								outDir, config.output.name, name)
// 						)
// 					)
// 			})
// 		)
